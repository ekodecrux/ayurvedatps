  ...
  try {
    ...
    const herbsRoute = await c.env.DB.prepare(`
      SELECT h.id, h.patient_id as patient_fk, h.appointment_id, h.diagnosis, h.notes, 
             ...
             p.additional_phones, p.medical_history
      FROM herbs_routes h
      LEFT JOIN patients p ON h.patient_id = p.id
      WHERE h.id = ?
    `).bind(id).first()
...
app.get('/api/backups/list', async (c) => {
  try {
    const response = await fetch('http://localhost:5000/api/backups/list')
    const data = await response.json()
    return c.json(data)
  } catch (error: any) {
    return c.json({ success: false, error: 'Backup API unavailable: ' + error.message }, 500)
  }
})
...
app.post('/api/backups/create', async (c) => {
  try {
    const response = await fetch('http://localhost:5000/api/backups/create', {
      method: 'POST',
...
app.post('/api/backups/restore', async (c) => {
  try {
    const body = await c.req.json()
    const response = await fetch('http://localhost:5000/api/backups/restore', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
// Health check for backup API
app.get('/api/backups/health', async (c) => {
  try {
...
// Legacy backup endpoint (redirect to list)
            ...
            <div id="appointments-section" class="section hidden">
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <input type="text" id="appointment-search" placeholder="Search by patient ID, name, or phone..." class="border rounded px-3 py-2" onkeyup="loadAppointments()">
            ...
            <div id="prescriptions-section" class="section hidden">
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <div class="mb-4 flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                        <input type="text" id="prescription-search" name="herbs-search-filter" autocomplete="off" placeholder="Search by patient ID, name, phone, or diagnosis..." class="border rounded px-3 py-2 flex-1" onkeyup="loadHerbsRoutes()">
                        <button onclick="exportToExcel()" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 flex items-center justify-center whitespace-nowrap">
                            <i class="fas fa-file-excel mr-2"></i>Export Excel
                        </button>
                        <button onclick="exportToPDF()" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 flex items-center justify-center whitespace-nowrap">
                ...
                <div id="summary-content" class="print-content">
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                            ...
                            <div class="col-span-2"><span class="font-semibold">Patient Name:</span> <span id="summary-patient-name"></span></div>
                            <div><span class="font-semibold">Age/Gender:</span> <span id="summary-patient-age-gender"></span></div>
                            <div class="col-span-2"><span class="font-semibold">Weight/Height:</span> <span id="summary-patient-weight-height"></span></div>
